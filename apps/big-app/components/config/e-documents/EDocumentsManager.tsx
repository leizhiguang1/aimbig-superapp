...
export function EDocumentsManager({ letterTemplates, formTemplates }: Props) {
	...
	const [editForm, setEditForm] = useState<FormTemplateWithSections | null>(
		null,
	);
	const [deleteForm, setDeleteForm] = useState<FormTemplateWithSections | null>(
		null,
	);
	const [formForm, setFormForm] = useState<FormFormState>({
		name: "",
		is_active: true,
		sections: [],
	});
	...
	const removeSection = (key: string) => {
		setFormForm((f) => ({
			...f,
			sections: f.sections.filter((s) => s._key !== key),
		}));
	};
		...
		<div className="space-y-6 p-6">
			<div>
				...
				<p className="text-muted-foreground text-sm">
					Manage letter templates and consent forms used across customer records
					and appointments.
				</p>
			</div>
			...
			{activeTab === "letters" && (
				<div className="space-y-4">
					<div className="flex items-center justify-between">
						<p className="text-muted-foreground text-sm">
							Reusable letter templates for referrals and other correspondence.
						</p>
					...
					{letterTemplates.length === 0 ? (
						<div className="rounded-lg border border-dashed p-12 text-center text-muted-foreground text-sm">
							No letter templates yet. Click &ldquo;Add Template&rdquo; to
			...
			{activeTab === "forms" && (
				<div className="space-y-4">
					<div className="flex items-center justify-between">
						<p className="text-muted-foreground text-sm">
							Consent forms and other document forms sent to customers.
						</p>
					...
					{formTemplates.length === 0 ? (
						<div className="rounded-lg border border-dashed p-12 text-center text-muted-foreground text-sm">
							No form templates yet. Click &ldquo;Add Form&rdquo; to create your
							first one.
						</div>
					) : (
						<div className="divide-y rounded-lg border">
			...
			<FormDialog
				...
				size="lg"
				title={editLetter ? "Edit Letter Template" : "New Letter Template"}
				footer={
					<>
						<Button
							variant="outline"
						...
						<Button
							...
							disabled={pending || !letterForm.name.trim()}
						>
							{pending ? (
			...
			>
				<div className="space-y-1.5">
					...
					<Input
						...
						onChange={(e) =>
							setLetterForm((f) => ({ ...f, name: e.target.value }))
						}
					/>
				</div>
				<div className="space-y-1.5">
					...
					<p className="text-muted-foreground text-xs">
						Click a merge field button in the toolbar to insert customer data
						automatically.
					</p>
					<RichTextEditor
						value={letterForm.body_html}
						onChange={(html) =>
							setLetterForm((f) => ({ ...f, body_html: html }))
						}
				...
				<div className="flex items-center gap-3">
					<Switch
						id="ltr-active"
						checked={letterForm.is_active}
						onCheckedChange={(v) =>
							setLetterForm((f) => ({ ...f, is_active: v }))
						}
					/>
					<Label htmlFor="ltr-active">
			...
			<FormDialog
				...
				size="xl"
				title={editForm ? "Edit Form Template" : "New Form Template"}
				...
				<div className="flex gap-4">
					<div className="flex-1 space-y-1.5">
						...
						<Input
							...
							value={formForm.name}
							onChange={(e) =>
								setFormForm((f) => ({ ...f, name: e.target.value }))
							}
						/>
					</div>
					<div className="flex items-end gap-2 pb-0.5">
						<Switch
							id="frm-active"
							checked={formForm.is_active}
							onCheckedChange={(v) =>
								setFormForm((f) => ({ ...f, is_active: v }))
							}
						/>
				...
				<div className="space-y-3">
					<div className="flex items-center justify-between">
						...
						<div className="flex gap-1.5">
							{(
								["open_text", "tnc", "signature", "checkbox"] as SectionType[]
							).map((type) => (
								<button
									key={type}
									type="button"
					...
					{formForm.sections.map((sec, idx) => (
						...
						>
								...
								<div className="flex items-center gap-3">
									<label className="flex items-center gap-1.5 text-xs text-muted-foreground cursor-pointer">
										<input
											...
											onChange={(e) =>
												updateSection(sec._key, { required: e.target.checked })
											}
											className="rounded"
										/>
										Required
							...
							<div className="space-y-1.5">
								...
								<Input
									placeholder={SECTION_TYPE_LABELS[sec.section_type]}
									value={sec.title}
									onChange={(e) =>
							...
							{(sec.section_type === "open_text" ||
								...
								<div className="space-y-1.5">
									...
									<RichTextEditor
										value={sec.body_html}
										onChange={(html) =>
											updateSection(sec._key, { body_html: html })
										}
							...
							{sec.section_type === "checkbox" && (
								<div className="space-y-1.5">
									...
									<Input
										placeholder="I agree to the above terms"
										value={sec.body_html}
										onChange={(e) =>
											updateSection(sec._key, { body_html: e.target.value })
										}
									/>
								</div>
							)}
			...
			{/* Confirm deletes */}
			<ConfirmDialog
				open={deleteLetter !== null}
				onOpenChange={(o) => !o && setDeleteLetter(null)}
				title="Delete this template?"
				description={`"${deleteLetter?.name}" will be removed. Existing letters are not affected.`}
				confirmLabel="Delete"
				pending={pending}
			...
			<ConfirmDialog
				...
				onOpenChange={(o) => !o && setDeleteForm(null)}
				title="Delete this form?"
				description={`"${deleteForm?.name}" will be removed. Existing responses are not affected.`}
				confirmLabel="Delete"
				pending={pending}
